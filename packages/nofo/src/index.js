...
export class NofoElement extends HTMLElement {
      ...
      Object.defineProperty(this, key, {
        get: () => {
          ...
          if (Array.isArray(defaultValue) || typeof defaultValue === 'object') {
            try { return attrVal ? JSON.parse(attrVal) : defaultValue; } catch { return defaultValue; }
          }
  ...
  enableTestMode(enabled = true) {
    ...
    if (enabled) {
      ...
      this.dispatchEvent(new CustomEvent(`${this.tagName.toLowerCase()}:testModeEnabled`, { bubbles: true, composed: true }));
    }
  }
  ...
  inject(key) {
    let res = null;
    this.dispatchEvent(new CustomEvent('nofo-context-request', {
  ...
  #interpolate(html, data) {
    ...
    result = result.replace(/\{\{(\w+(?:\.\w+)*)\}\}/g, (_, k) => this.#getNestedValue(data, k) ?? '');

    // 2. Conditional text: {{key ? truthy : falsy}}
    result = result.replace(/\{\{\s*(\w+(?:\.\w+)*)\s*\?\s*([^:}]+)\s*:\s*([^}]+)\s*\}\}/g, (_, k, truthy, falsy) => {
    ...
    result = result.replace(/nofo-(disabled|checked|hidden|readonly|required)="(\w+(?:\.\w+)*)"/g, (_, attr, k) => {
      const value = this.#getNestedValue(data, k);
      return value ? attr : '';
    });
    ...
    result = result.replace(/<(\w[\w-]*)([^>]*)\snofo-if="(\w+(?:\.\w+)*)"([^>]*)>/g, (match, tag, before, k, after) => {
      const value = this.#getNestedValue(data, k);
    ...
    result = result.replace(/<option([^>]*)nofo-selected-match="([^"]*)"([^>]*)>/g, (_, before, match, after) => {
  ...
  renderList(data, slot, options = {}) {
    if (!Array.isArray(data) || data.length === 0) {
      if (options.empty) {
        return typeof options.empty === 'function'
          ? options.empty()
          : options.empty;
      }
      return '';
  ...
  click() {
    const button = this.shadowRoot.querySelector('button') || this.shadowRoot.querySelector('[tabindex]') || this;
  ...
  focus() {
    const focusable = this.shadowRoot.querySelector('button, input, select, textarea, [tabindex]') || this;
    focusable?.focus?.();
    if (this.#isTestMode()) {
  ...
  #getThemeVars() {
    ...
    const knownVars = [
      'background', 'background-color', 'foreground', 'border', 'radius',
      'accent-primary', 'accent-secondary', 'spacing', 'shadow', 'font-family',
      'font-size', 'z-tooltip', 'z-modal', 'success', 'warning', 'destructive'
    ];
    knownVars.forEach(v => {
  ...
  is(state) {
    ...
    if (state === 'theme:dark') return this.getAttribute('theme') === 'dark' || this.dataset.theme === 'dark';
    if (state === 'theme:light') return this.getAttribute('theme') === 'light' || this.dataset.theme === 'light';
    return this.dataset[state] !== undefined;
  }

  hasTheme(themeName) {
    const theme = this.getAttribute('theme') || this.dataset.theme || this.getAttribute('color-mode');
