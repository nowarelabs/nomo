  ...
  styles() {
    ...
    return `
      :host {
        ...
        line-height: 1;
        box-sizing: border-box;
        ${Object.entries(variantStyles).map(([key, value]) => `${key.replace(/([A-Z])/g, '-$1').toLowerCase()}: ${value};`).join(' ')}
        ${Object.entries(sizeStyles).map(([key, value]) => `${key.replace(/([A-Z])/g, '-$1').toLowerCase()}: ${value};`).join(' ')}
      }
    `;
  }
}
