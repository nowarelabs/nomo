...
class NofoFlex extends NofoElement {
  ...
  onMount() {
    ...
    s.attr('align').toCSS('align-items', v => {
      const map = { start: 'flex-start', end: 'flex-end', center: 'center', baseline: 'baseline', stretch: 'stretch' };
      return map[v] || v;
    }).toDataAttr('align');
    s.attr('justify').toCSS('justify-content', v => {
      const map = { start: 'flex-start', end: 'flex-end', center: 'center', between: 'space-between' };
      return map[v] || v;
    }).toDataAttr('justify');
