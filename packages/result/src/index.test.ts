...
describe("Result", () => {
  ...
  describe("matchCode and matchStatus", () => {
    it("matchCode should match strings and wildcards", () => {
      ...
      if (typeof handlerMatch === "function")
        expect(handlerMatch()).toBe("handled_user_error");
      ...
      expect(matchCode("UNKNOWN", messageMapping, "Unknown status")).toBe(
        "Unknown status",
      );
    });
    ...
    it("matchCodeResult should return ok or err", () => {
      ...
      const apiResult = matchCodeResult(
        response.code,
        {
          ...
          "VALIDATION_*": () =>
            err("Validation failed", "Validation failed", 400, response.code),
          "AUTH_*": () =>
            err("Authentication failed", "Auth failed", 401, response.code),
          NOT_FOUND: () =>
            err("Resource not found", "Not found", 404, "NOT_FOUND"),
        },
        "Unknown error",
        "UNKNOWN_ERROR",
        500,
  ...
  describe("matching and tagging", () => {
    ...
    it("match should handle error tags in details", () => {
      ...
      const failure = err(
        { tag: "NOT_FOUND", id: 404 } as NotFoundError,
        "Missing",
        404,
      );
  ...
  describe("chaining methods", () => {
    ...
    it("recover should handle errors", () => {
      const result = err("fail").recover((e) =>
        ok(`recovered from ${e.error}`),
      );
      expect(result.success).toBe(true);
      ...
      const db = {
        findUserByEmail: async (email: string) => {
          if (email === "test@example.com")
            return { id: "1", email, hashedPassword: "hashed" };
          return null;
        },
      };
      const verifyPassword = async (p: string, h: string) =>
        p === "password123" && h === "hashed";
      ...
      const db = {
        findUser: async (id: string) => {
          ...
          if (id === "user-suspended")
            return { id, name: "Bob", status: "suspended" };
          return null;
        },
      };
