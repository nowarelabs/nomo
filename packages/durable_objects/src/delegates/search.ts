...
export class SearchDelegate extends DurableObjectBaseDelegate<SearchConfig> {
   ...
   */
  async handle(queryText: string, options: { limit?: number; offset?: number } = {}): Promise<any[]> {
    ...
    const results = await this.durableObject.storage.sql.exec(
      `SELECT * FROM ${table?.name || table} WHERE ${conditions} LIMIT ? OFFSET ?`,
      [...values, limit, offset]
    ).toArray();
  ...
  async paginate(page: number = 1, perPage: number = 20): Promise<{ data: any[]; total: number }> {
    ...
    const results = await this.durableObject.storage.sql.exec(
      `SELECT * FROM ${table?.name || table} LIMIT ? OFFSET ?`,
      [perPage, offset]
    ).toArray();
    
    const countRes = await this.durableObject.storage.sql.exec(
      `SELECT COUNT(*) as count FROM ${table?.name || table}`
    ).toArray();
