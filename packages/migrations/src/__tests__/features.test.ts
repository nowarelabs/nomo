...
describe("New Migration Features", () => {
  ...
  it("generates SQL for composite primary keys", async () => {
    ...
    const createCmd = commands.find((c: unknown) => c.up.type === "createTable");
  ...
  it("implements reflection-based changeColumn", async () => {
    ...
    const migration = new ChangeColumnMigration(mockDb as unknown);
    (migration as unknown)._inChange = true;
    const changeRes = await (migration as unknown).change();
    expect((changeRes as unknown)?.success ?? true).toBe(true);

    // Verify recreateTable was triggered
    const commands = (migration as unknown)._commands;
    const recreateCmd = commands.find((c: unknown) => c.up.type === "recreateTable");
  ...
  it("supports changeTable DSL for grouping modifications", async () => {
    ...
    const commands = (migration as unknown)._commands;
    expect(commands.some((c: unknown) => c.up.type === "addColumn")).toBe(true);
  ...
  it("supports reversible logic for custom up/down blocks", async () => {
    ...
    // Test Up
    (migration as unknown)._isUp = true;
    await migration.change();
    ...
    // Test Down
    (migration as unknown)._isUp = false;
    await migration.change();
