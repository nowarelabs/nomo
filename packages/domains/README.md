...
The Domain Orchestration layer for Noblackbox applications. It enables the implementation of complex, multi-step business transactions as cohesive "Domain" classes, providing a reliable execution loop with built-in hooks for auditing and error handling.

## Installation

```bash
pnpm add @noblackbox/domains
```
...
A "Domain" in Noblackbox is an aggregate root that orchestrates several underlying services or models to complete a high-level business objective. It focuses on the "what" and "when" of a process.
...
```typescript
import { BaseDomain } from "@noblackbox/domains";
...
- **Service Injection**: Domains should use services from `@noblackbox/services` to perform the actual work.
