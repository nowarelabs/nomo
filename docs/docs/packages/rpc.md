...
```typescript
import { BaseResourceRpcTarget } from "noware/rpc";
...
import { newWorkersRpcResponse } from "noware/rpc";

export class PostsRpcController extends BaseController<Env, ExecutionContext> {
  async rpc() {
    return newWorkersRpcResponse(this.request, new PostsRpc(this.request, this.env, this.ctx));
  }
}
```
