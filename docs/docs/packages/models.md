...
export class PostModel extends BaseModel<typeof posts, Post, NewPost> {
  ...
  async findByAuthor(authorId: string) {
    return this.query().where({ authorId }).orderBy("createdAt", "DESC").all();
  }
}
```
...
const postsWithComments = await postModel.query().where({ published: true }).with("comments").all();
```
...
```typescript
const postsWithAuthor = await postModel
  .query()
